...
def gen_proof(basepath: pathlib.Path, binary: pathlib.Path, proof: pathlib.Path, log: pathlib.Path, time_cost: pathlib.Path):
    ...
    postprocess = basepath / 'poc/veriwasm_proofgen/postprocess.py'
...
def run_checker(
    ...
    # ret = os.system(f'{checker} {binary} {proof} {disasm} {' '.join(opts)}')
    ret = subprocess.call([checker, binary, proof, disasm, cost_time] + opts)
    if ret != 0:
...
def main():
    parser = argparse.ArgumentParser(description='Generate proof/disasm and run checker for a given binary')
