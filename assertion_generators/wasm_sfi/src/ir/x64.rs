...
use crate::analyses::{reg_to_reg, write_proof_to_file};
use crate::ir::types::*;
use crate::ir::utils::{mk_value_i64, valsize};
use crate::loaders::types::{VwMetadata, VwModule};
use ptir::{
    unalias_location, Assignment, GenericMemCell, Imm, LocationBuilder, MemCell, MemCellBuilder,
    Proof,
};
...
pub fn lift(instr: &X64Instruction, addr: &Addr, metadata: &VwMetadata, strict: bool) -> Vec<Stmt> {
    ...
    match instr.opcode() {
        Opcode::MOV => {
            let unop = unop(Unopcode::Mov, instr);
            if let Stmt::Unop(_, dst, src) = unop {
                if let Value::Reg(dst_reg, size) = dst {
                    if let Operand::RegDisp(reg, disp) = instr.operand(1) {
                        if reg == RegSpec::rip() {
                            let target = addr + disp as u64 + instr.len();
                            let source = ptir::Location::Memory(
                                MemCellBuilder::new().displacement(target as i64).build(),
                            );
                            let proof = Proof::Asgn(Assignment {
                                lhs: unalias_location(
                                    LocationBuilder::new()
                                        .register(reg_to_reg(dst_reg, size).unwrap())
                                        .build(),
                                )
                                .unwrap(),
                                rhs: ptir::GenericExpr::Var(source),
                            });
                            write_proof_to_file(proof, *addr);
                        }
                    }
                }
            }
        ...
        Opcode::PUSH => {
            ...
            instrs.push(Stmt::Unop(
                ...
                Value::Mem(
                    ...
                    MemArgs::Mem2Args(MemArg::Reg(Rsp, Size64), MemArg::Imm(ImmType::Signed, Size64, -8i64)),
                ),
