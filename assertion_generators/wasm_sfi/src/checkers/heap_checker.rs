use crate::analyses::{output_proof_hint, AbstractAnalyzer, AnalysisResult, HeapAnalyzer};
use crate::checkers::Checker;
use crate::ir::types::{IRMap, MemArg, MemArgs, Stmt, ValSize, Value, X86Regs};
use crate::ir::utils::{is_frame_access, is_mem_access, is_stack_access};
use crate::lattices::heaplattice::{HeapLattice, HeapValue};
use crate::lattices::reachingdefslattice::LocIdx;
use crate::loaders::utils::is_libcall;
use ptir::Relationship;
...
impl Checker<HeapLattice> for HeapChecker<'_> {
    ...
    fn check_statement(&self, state: &HeapLattice, ir_stmt: &Stmt, loc_idx: &LocIdx) -> bool {
        match ir_stmt {
            ...
            Stmt::Unop(_, dst, src) => {
                if is_mem_access(dst) && !self.check_mem_access(state, dst, loc_idx, false) {
                    return false;
                }
                //stack read: probestack <= stackgrowth + c < 8K
                if is_mem_access(src) && !self.check_mem_access(state, src, loc_idx, true) {
                    return false;
                }
            }
            ...
            Stmt::Binop(_, dst, src1, src2) => {
                ...
                if is_mem_access(src1) && !self.check_mem_access(state, src1, loc_idx, true) {
                    return false;
                }
                if is_mem_access(src2) && !self.check_mem_access(state, src2, loc_idx, true) {
                    return false;
                }
            }
            Stmt::Clear(dst, srcs) => {
...
impl HeapChecker<'_> {
    ...
    fn check_ripconst_access(&self, state: &HeapLattice, access: &Value) -> bool {
        if let Value::Mem(_, memargs) = access {
            match memargs {
                // `RIPConst` represents a trusted value laoded from .rodata or
                // .data; any access involving such a pointer is trusted.
                //
                // An offset from the base, even with a computed value, is
                // acceptable here:
                //
                ...
                MemArgs::Mem2Args(MemArg::Reg(regnum, Size64), memarg2) => {
                    if let Some(HeapBase) = state.regs.get_reg(*regnum, Size64).v {
                        match memarg2 {
                            ...
                            MemArg::Imm(_, _, v) => {
                                log::debug!("check_heap_access: MemArg::Imm(_, _, v) = {:x?}", v);
    ...
    fn check_metadata_access(&self, state: &HeapLattice, access: &Value) -> bool {
        if let Value::Mem(_size, memargs) = access {
            match memargs {
                // TODO: this is a redundant check with check_globals_access, why is it here?
    ...
    fn check_mem_access(
        ...
        if is_stack_access(access) {
            output_proof_hint(format!("Stack{}", hint_str), None, loc_idx.addr);
            return true;
        }
        ...
        if self.check_ripconst_access(state, access) {
            output_proof_hint("RIPConstAccess".to_string(), None, loc_idx.addr);
            return true;
        }
        // Case 4: its a heap access
        if self.check_heap_access(state, access) {
            output_proof_hint(format!("Heap{}", hint_str), None, loc_idx.addr);
            return true;
        };
        // Case 5: its a metadata access
        if self.check_metadata_access(state, access) {
            output_proof_hint("MetaAccess".to_string(), None, loc_idx.addr);
            return true;
        };
        // Case 6: its a globals access
        if self.check_global_access(state, access) {
            output_proof_hint(format!("Global{}", hint_str), None, loc_idx.addr);
            return true;
        };
        // Case 7: Jump table access
        if self.check_jump_table_access(state, access) {
            output_proof_hint("JumpTableAccess".to_string(), None, loc_idx.addr);
            return true;
        };
        // Case 8: its unknown
