...
trait SmtVar: StringifySym + StringifySort {}
...
//     T: StringifySort + StringifySym + Sort2Smt + Sym2Smt + Hash,
{
        ...
        ssa.aliases.iter().try_for_each(|(loc, alias)| {
            ...
            if self.asserted_aliases.contains(&(*loc, alias.clone())) {
                ...
                let assertion = expr!(
                    loc.clone().into(),
                    bv!("="),
                    alias.get_sort().cast(loc.get_sort())?(alias.clone().into())
                );
    ...
    pub fn checked_assume(&mut self, assumptions: &Vec<SSExpr>) -> Result<()> {
        for r in assumptions {
            ...
            #[cfg(feature = "pushback")]
            // push the relationship back to the environment (not necessary!)
            {
                self.assume(r)?;
                self.relationships.push(r.clone());
            }
        }
        Ok(())
    }
    ...
    fn sandboxed_batch_check(
        ...
        Ok(assertions
            .iter()
            // get the results of all the sandboxed checks
            .map(|a| {
                self.debug_info(&a.1)?;
                self.sandboxed_check(&a.0, true, branch_conds)
            })
            .collect::<Result<Vec<bool>>>()?
            .iter()
            // only return true if all the checks are true
            .all(|b| *b))
    }
