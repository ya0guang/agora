...
impl SSAState {
        ...
        match loc {
            ...
            Location::Memory(m) => {
                let m_ssa = GenericMemCell {
                    ...
                    segment_reg: SubRegister::from(
                        m.segment_reg,
                        *self
                            .map
                            .get(&SSALocation::Register(m.segment_reg))
                            .unwrap_or(&0),
                    ),
    ...
    fn handle_stack_related(
        ...
        rhs_stack_uses.iter().for_each(|m| {
            ...
            warn!("Stack offset: {:x}", offset);
            warn!("Stack offset: {:?}", self.stack);
...
impl FuncSSA {
    ...
    pub fn get_initreg(&self, reg: Register) -> Option<LocationSub> {
        self.init_ssa.get_loc_ssa(&reg.into())
    }
}
...
fn interpret_bb<'a>(
    ...
    warn!("Interpreting BB 0x{:X}", _start_addr);
    ...
    for (addr, ins) in dis {
        log::debug!("Interpreting 0x{:X}: {:x?}", addr, ins);
