...
pub fn build_cfg(
    ...
    let _first_addr = dis.first_key_value().unwrap().0;
    for (addr, ins) in dis {
...
pub fn build_bbs(_raw: &[u8], dis: &Disassembled) -> Result<BTreeMap<u64, BasicBlock>> {
...
pub fn get_magic_sequences(
    path: &Path,
    dis: &HashMap<String, Disassembled>,
) -> Result<BTreeMap<u64, Vec<u8>>> {
    let mut magic = BTreeMap::new();
    ...
    for (name, block) in dis.iter() {
        ...
        let block_addr = block.keys().min().unwrap() - 16;
        println!("Block addr: {:0x?}", block_addr);
        ...
        for sym in obj_file.symbols() {
            ...
            let sym_addr = sym.address();
            if sym_addr == block_addr {
                println!("Symbol: {:?}", sym.name());
                if sym_section.name().unwrap() != ".text" {
                    continue; // only interested in text section symbols
                }
                let sym_bin = sym_section.data_range(sym_addr + 8, 8)?.unwrap();
                println!("Symbol: {:02x?}", sym_bin);
                magic.insert(sym_addr, sym_bin.to_vec());
            }
        }

        // for (sym_addr, sym_name) in sym_map.iter() {
        //     if *sym_addr == block_addr {
        //         println!("Symbol: {:?}", sym_name);
        //     }
        // }
        // println!("Block: {:?}", bin_data[block_addr as usize]);
    }
...
pub fn disasm_binary(path: &Path) -> Result<HashMap<String, Disassembled>> {
    ...
    let functions = obj_file.symbols().filter(|sym| {
        sym.kind() == SymbolKind::Text || sym.name().unwrap().starts_with("__sgx_function_magic_")
    });
    ...
    for sym in functions {
        ...
        // println!("Section: {:?}", sym_section);
        let sym_addr = sym.address();
        let sym_size = match sym.name().unwrap().starts_with("__sgx_function_magic_") {
            // true => 16,
            _ => sym.size(),
        };
        if sym_size == 0 {
