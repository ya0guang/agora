...
impl Encoder {
    pub(super) const ERROR_ONLY_1632_BIT_MODE: &'static str =
        "The instruction can only be used in 16/32-bit mode";
    pub(super) const ERROR_ONLY_64_BIT_MODE: &'static str =
        "The instruction can only be used in 64-bit mode";
    ...
    pub fn encode(&mut self, instruction: &Instruction, rip: u64) -> Result<usize, IcedError> {
        ...
        if instr_len > IcedConstants::MAX_INSTRUCTION_LENGTH && !handler.is_special_instr {
            self.set_error_message(format!(
                ...
                IcedConstants::MAX_INSTRUCTION_LENGTH
            ));
        }
    ...
    #[must_use]
    #[inline]
    pub(super) fn verify_register_range(
        &mut self,
        operand: u32,
        register: Register,
        reg_lo: Register,
    ...
    pub(super) fn set_addr_size(&mut self, reg_size: u32) {
        ...
        if self.bitness == 64 {
            if reg_size == 2 {
                self.set_error_message(format!(
                    "Invalid register size: {}, must be 32-bit or 64-bit",
                    reg_size.wrapping_mul(8)
                ));
            } else if reg_size == 4 {
            ...
            if reg_size == 8 {
                self.set_error_message(format!(
                    "Invalid register size: {}, must be 16-bit or 32-bit",
                    reg_size.wrapping_mul(8)
                ));
    ...
    pub(super) fn add_abs_mem(&mut self, instruction: &Instruction, operand: u32) {
        ...
        if op_kind == OpKind::Memory {
            ...
            {
                self.set_error_message(format!(
                    "Operand {}: Absolute addresses can't have base and/or index regs",
                    operand
                ));
                return;
            }
            if instruction.memory_index_scale() != 1 {
                self.set_error_message(format!(
                    "Operand {}: Absolute addresses must have scale == *1",
                    operand
                ));
                return;
            }
            ...
            if cfg!(debug_assertions) {
                self.set_error_message(format!(
                    "Operand {}: Expected OpKind::Memory, actual: {:?}",
                    operand, op_kind
                ));
            } else {
                self.set_error_message(format!(
                    "Operand {}: Expected OpKind::Memory, actual: OpKind value {}",
    ...
    pub(super) fn add_mod_rm_register(
        &mut self,
        instruction: &Instruction,
        operand: u32,
        reg_lo: Register,
    ...
    pub(super) fn add_reg_or_mem_full(
        ...
        reg_lo: Register,
        reg_hi: Register,
        vsib_index_reg_lo: Register,
        vsib_index_reg_hi: Register,
        allow_mem_op: bool,
        allow_reg_op: bool,
    ) {
        let op_kind = instruction.op_kind(operand);
        ...
        if op_kind == OpKind::Register {
            if !allow_reg_op {
                self.set_error_message(format!(
                    "Operand {}: register operand is not allowed",
                    operand
                ));
                return;
            }
            ...
            if !allow_mem_op {
                self.set_error_message(format!(
                    "Operand {}: memory operand is not allowed",
                    operand
                ));
                return;
            }
    ...
    fn add_mem_op16(&mut self, instruction: &Instruction, operand: u32) {
        if self.bitness == 64 {
            self.set_error_message(format!(
                "Operand {}: 16-bit addressing can't be used by 64-bit code",
                operand
            ));
            return;
        }
        ...
        if base == Register::BX && index == Register::SI {
            ...
            if instruction.memory_displacement64() > u16::MAX as u64 {
                self.set_error_message(format!(
                    "Operand {}: Displacement must fit in a u16",
                    operand
                ));
                return;
            }
        ...
        if base != Register::None || index != Register::None {
            if (instruction.memory_displacement64() as i64) < i16::MIN as i64
                || (instruction.memory_displacement64() as i64) > u16::MAX as i64
            {
                self.set_error_message(format!(
                    "Operand {}: Displacement must fit in an i16 or a u16",
                    operand
                ));
                return;
            }
            ...
            if displ_size == 0 && base == Register::BP && index == Register::None {
                ...
                if self.displ != 0 {
                    self.set_error_message(format!(
                        "Operand {}: Displacement must be 0 if displ_size == 0",
                        operand
                    ));
                    return;
                }
            ...
            if displ_size == 1 {
                if let Some(compressed_value) =
                    self.try_convert_to_disp8n(instruction, self.displ as i16 as i32)
                {
            ...
            if displ_size == 0 {
                ...
                if (self.displ as i32) < i8::MIN as i32 || (self.displ as i32) > i8::MAX as i32 {
                    self.set_error_message(format!(
                        "Operand {}: Displacement must fit in an i8",
                        operand
                    ));
                    return;
                }
    ...
    fn add_mem_op(
        ...
        if self.bitness != 64 && addr_size == 64 {
            self.set_error_message(format!(
                "Operand {}: 64-bit addressing can only be used in 64-bit mode",
                operand
            ));
            return;
        }
        ...
        if base != Register::None
            && base != Register::RIP
            && base != Register::EIP
            && !self.verify_register_range(operand, base, base_lo, base_hi)
        {
            return;
        }
        if index != Register::None
            && !self.verify_register_range(operand, index, index_lo, index_hi)
        {
            return;
        }
        ...
        if displ_size != 0 && displ_size != 1 && displ_size != 4 && displ_size != 8 {
            self.set_error_message(format!(
                "Operand {}: Invalid displ size: {}, must be 0, 1, 4, 8",
                operand, displ_size
            ));
            return;
        }
        if base == Register::RIP || base == Register::EIP {
            if index != Register::None {
                self.set_error_message(format!(
                    "Operand {}: RIP relative addressing can't use an index register",
                    operand
                ));
                return;
            }
            if instruction_internal::internal_get_memory_index_scale(instruction) != 0 {
                self.set_error_message(format!(
                    "Operand {}: RIP relative addressing must use scale *1",
                    operand
                ));
                return;
            }
            ...
            if (self.encoder_flags & EncoderFlags::MUST_USE_SIB) != 0 {
                self.set_error_message(format!(
                    "Operand {}: RIP/EIP relative addressing isn't supported",
                    operand
                ));
                return;
            }
            ...
            if base == Register::RIP {
                ...
                if target > u32::MAX as u64 {
                    self.set_error_message(format!(
                        "Operand {}: Target address doesn't fit in 32 bits: 0x{:X}",
                        operand, target
                    ));
                    return;
                }
        ...
        self.displ = instruction.memory_displacement32();
        if addr_size == 64 {
            if (instruction.memory_displacement64() as i64) < i32::MIN as i64
                || (instruction.memory_displacement64() as i64) > i32::MAX as i64
            {
                self.set_error_message(format!(
                    "Operand {}: Displacement must fit in an i32",
                    operand
                ));
                return;
            }
            ...
            debug_assert_eq!(addr_size, 32);
            if (instruction.memory_displacement64() as i64) < i32::MIN as i64
                || (instruction.memory_displacement64() as i64) > u32::MAX as i64
            {
                self.set_error_message(format!(
                    "Operand {}: Displacement must fit in an i32 or a u32",
                    operand
                ));
                return;
            }
        ...
        if base == Register::None && index == Register::None {
            if vsib_index_reg_lo != Register::None {
                self.set_error_message(format!(
                    "Operand {}: VSIB addressing can't use an offset-only address",
                    operand
                ));
                return;
            }
            if self.bitness == 64
                || scale != 0
                || (self.encoder_flags & EncoderFlags::MUST_USE_SIB) != 0
            {
                self.mod_rm |= 4;
                self.displ_size = DisplSize::Size4;
        ...
        if displ_size == 1 {
            if let Some(compressed_value) =
                self.try_convert_to_disp8n(instruction, self.displ as i32)
            {
                self.displ = compressed_value as u32;
            } else {
            ...
            && (self.encoder_flags & EncoderFlags::MUST_USE_SIB) == 0
        {
            ...
            if index == Register::RSP || index == Register::ESP {
                self.set_error_message(format!(
                    "Operand {}: ESP/RSP can't be used as an index register",
                    operand
                ));
                return;
            }
    ...
    fn write_prefixes(&mut self, instruction: &Instruction, can_write_f3: bool) {
        ...
        if seg != Register::None {
            ...
            debug_assert!(
                (seg as usize).wrapping_sub(Register::ES as usize) < SEGMENT_OVERRIDES.len()
            );
            ...
            self.write_byte_internal(unsafe {
                *SEGMENT_OVERRIDES.get_unchecked((seg as usize).wrapping_sub(Register::ES as usize))
            } as u32);
        }
        ...
        match self.displ_size {
            ...
            DisplSize::RipRelSize4_Target32 => {
                let eip = (self.current_rip as u32)
                    .wrapping_add(4)
                    .wrapping_add(IMM_SIZES[self.imm_size as usize]);
            ...
            DisplSize::RipRelSize4_Target64 => {
                let rip = self
                    .current_rip
                    .wrapping_add(4)
                    .wrapping_add(IMM_SIZES[self.imm_size as usize] as u64);
                let diff8 =
                    ((((self.displ_hi as u64) << 32) | self.displ as u64).wrapping_sub(rip)) as i64;
    ...
    fn write_immediate(&mut self) {
        ...
        match self.imm_size {
            ...
            ImmSize::RipRelSize1_Target64 => {
                ...
                diff8 = (((self.immediate_hi as u64) << 32) | (self.immediate as u64))
