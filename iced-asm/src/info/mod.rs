...
impl UsedMemory {
        ...
        F: FnMut(Register, usize, usize) -> Option<u64>,
    {
        ...
        match self.index {
            ...
            _ => {
                let mut index =
                    get_register_value(self.index, element_index, self.vsib_size as usize)?;
                if self.vsib_size == 4 {
...
impl InstructionInfo {
    ...
    pub fn try_op_access(&self, operand: u32) -> Result<OpAccess, IcedError> {
        self.op_accesses.get(operand as usize).map_or_else(
            || Err(IcedError::new("Invalid operand")),
            |&op_access| Ok(op_access),
        )
    }
}
