...
impl InstructionInfoFactory {
    ...
    fn create<'a>(
        ...
        const _: () =
            assert!(InstructionInfoOptions::NO_REGISTER_USAGE == Flags::NO_REGISTER_USAGE);
        ...
        let op1_info: OpInfo1 = unsafe {
            mem::transmute(
                ((flags1 >> InfoFlags1::OP_INFO1_SHIFT) & InfoFlags1::OP_INFO1_MASK) as u8,
            )
        };
        info.op_accesses[1] = OP_ACCESS_1[op1_info as usize];
        let op2_info: OpInfo2 = unsafe {
            mem::transmute(
        ...
        for i in 0..(instruction.op_count() as usize) {
            ...
            match instruction.op_kind(i as u32) {
                        ...
                        if i == 0 && op0_info == OpInfo0::WriteForceP1 {
                            ...
                            if Register::K0 <= reg && reg <= Register::K7 {
                                ...
                                Self::add_register(
                                    ...
                                    unsafe {
                                        mem::transmute(
                                            (((reg as u32 - Register::K0 as u32) ^ 1)
                                                + Register::K0)
                            ...
                            if Register::XMM0 <= reg && reg <= IcedConstants::VMM_LAST {
                                ...
                                let reg_base = (IcedConstants::VMM_FIRST as u32).wrapping_add(
                                    (reg as u32).wrapping_sub(IcedConstants::VMM_FIRST as u32) & !3,
                                );
                                for j in 0..4 {
                                    Self::add_register(
                                        flags,
                                        info,
                                        unsafe {
                                            mem::transmute((reg_base + j) as RegisterUnderlyingType)
                                        },
                                        access,
                                    );
                                }
                            ...
                            Self::add_register(
                                flags,
                                info,
                                instruction.op_register(i as u32),
                                access,
                            );
                        }
                    }
                ...
                OpKind::Memory => {
                    ...
                    let segment_register = unsafe {
                        mem::transmute(
                            (instruction.memory_segment() as u32 & !((flags1 as i32 >> 31) as u32))
                                as RegisterUnderlyingType,
                        )
                    };
                    ...
                    if base_register == Register::RIP {
                        ...
                        if (flags & Flags::NO_REGISTER_USAGE) == 0
                            && segment_register != Register::None
                        {
                            Self::add_memory_segment_register(
                                flags,
                                info,
                                segment_register,
                                OpAccess::Read,
                        ...
                        let (index_register, scale) = if (flags1 & InfoFlags1::IGNORES_INDEX_VA)
                            != 0
                        {
                        ...
                        if (flags & Flags::NO_MEMORY_USAGE) == 0 {
                            ...
                            let displ = if addr_size_bytes == 8 {
                                instruction.memory_displacement64()
                            } else {
                                instruction.memory_displacement32() as u64
                            };
        ...
        let implied_access = unsafe {
            mem::transmute(
                ((flags1 >> InfoFlags1::IMPLIED_ACCESS_SHIFT) & InfoFlags1::IMPLIED_ACCESS_MASK)
                    as u8,
            )
        };
        ...
        if instruction.has_op_mask() && (flags & Flags::NO_REGISTER_USAGE) == 0 {
            Self::add_register(
                ...
                info,
                instruction.op_mask(),
                if (flags1 & InfoFlags1::OP_MASK_READ_WRITE) != 0 {
                    OpAccess::ReadWrite
                } else {
                    OpAccess::Read
                },
        ...
        if (flags & Flags::NO_MEMORY_USAGE) == 0 {
            ...
            if instruction.op0_kind() == OpKind::Memory {
                    ...
                    || instruction.memory_base() == Register::ESP
                {
                    ...
                    info.used_memory_locations[0] = UsedMemory {
                        displacement: displ,
                        ..mem
                    };
                }
        ...
        for i in 0..8 {
            if (flags & Flags::NO_REGISTER_USAGE) == 0 {
                Self::add_register(
                    ...
                    unsafe {
                        mem::transmute(
                            (base_register as u32).wrapping_add(i) as RegisterUnderlyingType
                        )
                    },
                    OpAccess::Read,
                );
            ...
            if i != 3 {
                if (flags & Flags::NO_REGISTER_USAGE) == 0 {
                    Self::add_register(
                        ...
                        info,
                        unsafe {
                            mem::transmute((base_register as u32).wrapping_add(7).wrapping_sub(i)
                                as RegisterUnderlyingType)
                        },
                        OpAccess::Write,
                    );
    ...
    fn command_enter(
        ...
        if (flags & Flags::NO_REGISTER_USAGE) == 0 {
            Self::add_register(
                flags,
                info,
                unsafe { mem::transmute((r_sp as u32).wrapping_add(1) as RegisterUnderlyingType) },
                OpAccess::ReadWrite,
            );
        }
        if (flags & Flags::NO_MEMORY_USAGE) == 0 {
        ...
        if nesting_level != 0 {
            let xbp =
                unsafe { mem::transmute((xsp as u32).wrapping_add(1) as RegisterUnderlyingType) }; // rBP immediately follows rSP
            let mut xbp_offset = 0u64;
            for i in 1..(nesting_level as u32) {
                if i == 1
                    && r_sp as u32 + 1 != xbp as u32
                    && (flags & Flags::NO_REGISTER_USAGE) == 0
                {
        ...
        if op_size == 8 {
            if (flags & Flags::NO_MEMORY_USAGE) == 0 {
                Self::add_memory(
                    ...
                    Register::SS,
                    unsafe {
                        mem::transmute((xsp as u32).wrapping_add(1) as RegisterUnderlyingType)
                    },
                    Register::None,
                    1,
                    0,
    ...
    fn is_clear_instr(instruction: &Instruction) -> bool {
        ...
        {
            matches!(
                instruction.mvex_reg_mem_conv(),
                MvexRegMemConv::None | MvexRegMemConv::RegSwizzleNone
            )
        }
    ...
    fn command_clear_reg_regmem(instruction: &Instruction, info: &mut InstructionInfo, flags: u32) {
        if instruction.op0_register() == instruction.op1_register()
            && instruction.op1_kind() == OpKind::Register
            && Self::is_clear_instr(instruction)
        {
            info.op_accesses[0] = OpAccess::Write;
            info.op_accesses[1] = OpAccess::None;
            if (flags & Flags::NO_REGISTER_USAGE) == 0 {
    ...
    fn command_clear_reg_reg_regmem(
        instruction: &Instruction,
        info: &mut InstructionInfo,
        flags: u32,
    ) {
        if instruction.op1_register() == instruction.op2_register()
            ...
            && Self::is_clear_instr(instruction)
        {
            ...
            if (flags & Flags::NO_REGISTER_USAGE) == 0 {
                ...
                debug_assert_eq!(
                    info.used_registers[info.used_registers.len() - 2].register(),
                    instruction.op1_register()
                );
                debug_assert_eq!(
                    info.used_registers[info.used_registers.len() - 1].register(),
    ...
    fn command_arpl(instruction: &Instruction, info: &mut InstructionInfo, flags: u32) {
        if (flags & Flags::NO_REGISTER_USAGE) == 0 {
            ...
            let start_index = if instruction.op0_kind() == OpKind::Register {
                0
            } else {
            ...
            for info in &mut info.used_registers[start_index..] {
                ...
                if index >= 0 {
                    info.register = unsafe {
                        mem::transmute((Register::AL as u32).wrapping_add(index as u32)
                            as RegisterUnderlyingType)
                    };
                }
            }
    ...
    fn command_last_gpr(
        ...
        if (flags & Flags::NO_REGISTER_USAGE) == 0 {
            ...
            if instruction.op_kind(op_index) == OpKind::Register {
                ...
                debug_assert_eq!(
                    instruction.op_register(op_index),
                    info.used_registers[info.used_registers.len() - N].register()
                );
                debug_assert_eq!(
                    info.used_registers[info.used_registers.len() - N].access(),
                    OpAccess::Read
                );
                ...
                if index >= 0 {
                    ...
                    info.used_registers[regs_index] = UsedRegister {
                        register: unsafe {
                            mem::transmute((base_reg as u32).wrapping_add(index as u32)
                                as RegisterUnderlyingType)
                        },
                        access: OpAccess::Read,
                    };
                ...
                if reg >= Register::EAX && reg <= Register::R15D {
                    if reg_info.register() >= Register::RAX && reg_info.register() <= Register::R15
                    {
                        reg_info.register = unsafe {
                            mem::transmute(
                                (reg_info.register() as u32)
                                    .wrapping_sub(Register::RAX as u32)
                                    .wrapping_add(Register::EAX as u32)
                                    as RegisterUnderlyingType,
                            )
                        };
                    }
                    ...
                    if reg_info.register() >= Register::EAX && reg_info.register() <= Register::R15
                    {
                        reg_info.register = unsafe {
                            mem::transmute(
                                ((reg_info.register() as u32).wrapping_sub(Register::EAX as u32)
                                    & 0xF)
                                    .wrapping_add(Register::AX as u32)
                                    as RegisterUnderlyingType,
                            )
                        };
                    }
        ...
        if (flags & Flags::NO_REGISTER_USAGE) == 0 {
            if instruction.op0_kind() == OpKind::Register {
                ...
                if reg >= Register::MM0 && reg <= Register::MM7 {
                    reg = unsafe {
                        mem::transmute(
                            (((reg as u32 - Register::MM0 as u32) ^ 1) + Register::MM0 as u32)
                                as RegisterUnderlyingType,
                        )
                    };
        ...
        if (flags & Flags::NO_MEMORY_USAGE) == 0 {
            if info.used_memory_locations.len() == 1 {
                if let Some(loc) = info.used_memory_locations.get_mut(0) {
                    ...
                    const _: () = assert!(CodeSize::Code32 as u32 == 2);
                    const _: () = assert!(CodeSize::Code64 as u32 == 3);
                    loc.displacement = loc.displacement.wrapping_add(displ as u64)
                        & MASK[loc.address_size as usize];
                } else {
                    debug_assert!(false);
                }
            } else {
    ...
    fn add_memory(
        ...
        access: OpAccess,
        mut address_size: CodeSize,
        vsib_size: u32,
    ) {
        if address_size == CodeSize::Unknown {
            let reg = if base_register != Register::None {
                base_register
            } else {
                index_register
            };
    ...
    #[inline(always)]
    fn add_memory_segment_register(
        flags: u32,
        info: &mut InstructionInfo,
        seg: Register,
        access: OpAccess,
    ...
    fn add_register(flags: u32, info: &mut InstructionInfo, mut reg: Register, access: OpAccess) {
        ...
        if (flags & (Flags::IS_64BIT | Flags::ZERO_EXT_VEC_REGS)) != 0 {
            ...
            if (access as u32).wrapping_sub(OpAccess::Write as u32) <= 3 {
                ...
                if (flags & Flags::IS_64BIT) != 0
                    && index <= (Register::R15D as u32 - Register::EAX as u32)
                {
                    write_reg = unsafe {
                        mem::transmute(
                            (Register::RAX as u32).wrapping_add(index) as RegisterUnderlyingType
                        )
                    };
                } else {
                    index = (reg as u32).wrapping_sub(Register::XMM0 as u32);
                    if (flags & Flags::ZERO_EXT_VEC_REGS) != 0
                        && index <= IcedConstants::VMM_LAST as u32 - Register::XMM0 as u32
                    {
                        write_reg = unsafe {
                            mem::transmute(
                                (Register::ZMM0 as u32)
                                    .wrapping_add(index % IcedConstants::VMM_COUNT)
                                    as RegisterUnderlyingType,
                            )
                        };
                    }
