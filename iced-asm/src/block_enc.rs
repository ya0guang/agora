...
impl BlockEncInt {
    ...
    fn get_target(&self, base: &InstrBase, address: u64) -> TargetInstr {
        if (address != 0 || !self.has_multiple_zero_ip_instrs) && base.orig_ip == address {
            ...
            match self
                .to_instr_index
                .binary_search_by(|(ip, _)| address.cmp(ip))
            {
    ...
    fn get_instruction_size(&mut self, instruction: &Instruction, ip: u64) -> u32 {
        ...
        self.null_encoder.encode(instruction, ip).map_or_else(
            |_| IcedConstants::MAX_INSTRUCTION_LENGTH as u32,
            |len| len as u32,
        )
    }
}
...
impl BlockEncoder {
    fn new(
        bitness: u32,
        instr_blocks: &[InstructionBlock<'_>],
        options: u32,
        ...
        for (ip, _) in this.benc.to_instr_index.iter().skip(1) {
            if *ip == prev_ip {
                return Err(IcedError::with_string(format!(
                    "Multiple instructions with the same IP: 0x{:X}",
                    ip
                )));
            }
            prev_ip = *ip;
    ...
    pub fn encode(
        ...
        options: u32,
    ) -> Result<BlockEncoderResult, IcedError> {
    ...
    pub fn encode_slice(
        bitness: u32,
        blocks: &[InstructionBlock<'_>],
        options: u32,
    ) -> Result<Vec<BlockEncoderResult>, IcedError> {
    ...
    fn encode2(&mut self) -> Result<Vec<BlockEncoderResult>, IcedError> {
        ...
        for _ in 0..5 {
            ...
            for info in &mut self.blocks {
                ...
                let mut ctx = InstrContext {
                    block: &mut info.0,
                    all_ips: &mut self.all_ips,
                    ip: block_rip,
                };
        ...
        for info in &mut self.blocks {
            ...
            let mut constant_offsets: Vec<ConstantOffsets> =
                if (self.benc.options & BlockEncoderOptions::RETURN_CONSTANT_OFFSETS) != 0 {
                    Vec::with_capacity(instr_count)
                } else {
                    Vec::new()
                };
            ...
            for (base, instr) in &mut self.all_instrs[info.1..info.2] {
                ...
                if (self.benc.options & BlockEncoderOptions::RETURN_NEW_INSTRUCTION_OFFSETS) != 0 {
                    new_instruction_offsets.push(if is_original_instruction {
