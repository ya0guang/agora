...
impl<'a, 'b> OpCodeFormatter<'a, 'b> {
    ...
    fn has_mod_rm(&self) -> bool {
        ...
        match self.op_code.encoding() {
            ...
            EncodingKind::EVEX | EncodingKind::XOP | EncodingKind::D3NOW | EncodingKind::MVEX => {
                return true
            }
        }
    ...
    fn get_op_code_bits_operand(&self) -> OpCodeOperandKind {
        for &op_kind in self.op_code.op_kinds() {
            match op_kind {
                ...
                | OpCodeOperandKind::r64_opcode => return op_kind,
                _ => {}
            }
        }
        ...
        if !self.has_modrm_info {
            ...
            for &op_kind in self.op_code.op_kinds() {
                match op_kind {
                    ...
                    | OpCodeOperandKind::mem
                    | OpCodeOperandKind::mem_mpx
                    | OpCodeOperandKind::mem_mib => is_reg_only = false,
                    OpCodeOperandKind::mem_vsib32x
                    | OpCodeOperandKind::mem_vsib64x
                    | OpCodeOperandKind::mem_vsib32y
                    | OpCodeOperandKind::mem_vsib64y
    ...
    fn append_rest(&mut self) {
        if let Some((is_reg_only, rrr, bbb)) = self.get_modrm_info() {
            self.sb
                .push_str(if is_reg_only { " 11:" } else { " !(11):" });
            self.append_bits("rrr", rrr, 3);
            self.sb.push(':');
            self.append_bits("bbb", bbb, 3);
        } else {
            let is_vsib = (self.op_code.encoding() == EncodingKind::EVEX
                || self.op_code.encoding() == EncodingKind::MVEX)
                && self.has_vsib();
        ...
        for &op_kind in self.op_code.op_kinds() {
            match op_kind {
                ...
                | OpCodeOperandKind::br64_1 => self.sb.push_str(" cb"),
                OpCodeOperandKind::br16_2
                | OpCodeOperandKind::xbegin_2
                | OpCodeOperandKind::brdisp_2 => self.sb.push_str(" cw"),
                ...
                | OpCodeOperandKind::imm8sex64 => self.sb.push_str(" ib"),
                ...
                OpCodeOperandKind::xmm_is4 | OpCodeOperandKind::ymm_is4 => {
                    self.sb.push_str(" /is4")
                }
