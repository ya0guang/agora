...
pub(crate) struct OpCodeHandler {
    pub(super) encode:
        fn(self_ptr: *const OpCodeHandler, encoder: &mut Encoder, instruction: &Instruction),
    pub(super) try_convert_to_disp8n: Option<
        fn(
            self_ptr: *const OpCodeHandler,
            encoder: &mut Encoder,
...
impl LegacyHandler {
    pub(super) fn new(enc_flags1: u32, enc_flags2: u32, enc_flags3: u32) -> Self {
        let group_index = if (enc_flags2 & EncFlags2::HAS_GROUP_INDEX) == 0 {
            -1
        } else {
            ((enc_flags2 >> EncFlags2::GROUP_INDEX_SHIFT) & 7) as i32
        };
        let rm_group_index = if (enc_flags3 & EncFlags3::HAS_RM_GROUP_INDEX) == 0 {
            -1
        } else {
            ((enc_flags2 >> EncFlags2::GROUP_INDEX_SHIFT) & 7) as i32
        };
        // SAFETY: generated data is valid
        let table: LegacyOpCodeTable = unsafe {
            mem::transmute(((enc_flags2 >> EncFlags2::TABLE_SHIFT) & EncFlags2::TABLE_MASK) as u8)
        };
        ...
        // SAFETY: generated data is valid
        let mpb: MandatoryPrefixByte = unsafe {
            mem::transmute(
                ((enc_flags2 >> EncFlags2::MANDATORY_PREFIX_SHIFT)
                    & EncFlags2::MANDATORY_PREFIX_MASK) as u8,
            )
        };
        ...
        let op0 =
            ((enc_flags1 >> EncFlags1::LEGACY_OP0_SHIFT) & EncFlags1::LEGACY_OP_MASK) as usize;
        let op1 =
            ((enc_flags1 >> EncFlags1::LEGACY_OP1_SHIFT) & EncFlags1::LEGACY_OP_MASK) as usize;
        let op2 =
            ((enc_flags1 >> EncFlags1::LEGACY_OP2_SHIFT) & EncFlags1::LEGACY_OP_MASK) as usize;
        let op3 =
            ((enc_flags1 >> EncFlags1::LEGACY_OP3_SHIFT) & EncFlags1::LEGACY_OP_MASK) as usize;
        ...
        Self {
            base: OpCodeHandler {
                ...
                op_size: unsafe {
                    mem::transmute(
                        ((enc_flags3 >> EncFlags3::OPERAND_SIZE_SHIFT)
                            & EncFlags3::OPERAND_SIZE_MASK)
                            as CodeSizeUnderlyingType,
                    )
                },
                // SAFETY: generated data is valid
                addr_size: unsafe {
                    mem::transmute(
                        ((enc_flags3 >> EncFlags3::ADDRESS_SIZE_SHIFT)
                            & EncFlags3::ADDRESS_SIZE_MASK)
                            as CodeSizeUnderlyingType,
                    )
                },
...
impl VexHandler {
    pub(super) fn new(enc_flags1: u32, enc_flags2: u32, enc_flags3: u32) -> Self {
        ...
        // SAFETY: generated data is valid
        let wbit: WBit = unsafe {
            mem::transmute(((enc_flags2 >> EncFlags2::WBIT_SHIFT) & EncFlags2::WBIT_MASK) as u8)
        };
        let w1 = if wbit == WBit::W1 { u32::MAX } else { 0 };
        // SAFETY: generated data is valid
        let lbit: LBit = unsafe {
            mem::transmute(((enc_flags2 >> EncFlags2::LBIT_SHIFT) & EncFlags2::LBIT_MASK) as u8)
        };
        let mut last_byte = if lbit == LBit::L1 || lbit == LBit::L256 {
            4
        } else {
            0
        };
        ...
        last_byte |=
            (enc_flags2 >> EncFlags2::MANDATORY_PREFIX_SHIFT) & EncFlags2::MANDATORY_PREFIX_MASK;
    ...
    fn encode(self_ptr: *const OpCodeHandler, encoder: &mut Encoder, instruction: &Instruction) {
        ...
        if (encoder.prevent_vex2
            | this.w1
            | this.table.wrapping_sub(VexOpCodeTable::MAP0F as u32)
...
impl EvexHandler {
    pub(super) fn new(enc_flags1: u32, enc_flags2: u32, enc_flags3: u32) -> Self {
        ...
        let mut p1_bits = 4
            | ((enc_flags2 >> EncFlags2::MANDATORY_PREFIX_SHIFT)
                & EncFlags2::MANDATORY_PREFIX_MASK);
        // SAFETY: generated data is valid
        let wbit: WBit = unsafe {
            mem::transmute(((enc_flags2 >> EncFlags2::WBIT_SHIFT) & EncFlags2::WBIT_MASK) as u8)
        };
        ...
        Self {
            ...
            tuple_type: unsafe {
                mem::transmute(
                    ((enc_flags3 >> EncFlags3::TUPLE_TYPE_SHIFT) & EncFlags3::TUPLE_TYPE_MASK)
                        as TupleTypeUnderlyingType,
                )
            },
            wbit,
        }
    ...
    fn try_convert_to_disp8n(
        ...
        let n = get_disp8n(
            this.tuple_type,
            (encoder.encoder_flags & EncoderFlags::BROADCAST) != 0,
        ) as i32;
        let res = displ / n;
    ...
    fn encode(self_ptr: *const OpCodeHandler, encoder: &mut Encoder, instruction: &Instruction) {
        ...
        if instruction.suppress_all_exceptions() {
            if (this.base.enc_flags3 & EncFlags3::SUPPRESS_ALL_EXCEPTIONS) == 0 {
                encoder.set_error_message_str(
                    "The instruction doesn't support suppress-all-exceptions",
                );
            }
            b |= 0x10;
        }
        ...
        } else if (this.base.enc_flags3 & EncFlags3::SUPPRESS_ALL_EXCEPTIONS) == 0
...
impl MvexHandler {
    ...
    fn try_convert_to_disp8n(
        ...
        let tuple_type = crate::mvex::mvex_tt_lut::MVEX_TUPLE_TYPE_LUT
            [(mvex.tuple_type_lut_kind as usize) * 8 + sss];
    ...
    fn encode(self_ptr: *const OpCodeHandler, encoder: &mut Encoder, instruction: &Instruction) {
        ...
        {
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 1 == MvexRegMemConv::MemConvBroadcast1 as u32
            );
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 2 == MvexRegMemConv::MemConvBroadcast4 as u32
            );
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 3 == MvexRegMemConv::MemConvFloat16 as u32
            );
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 4 == MvexRegMemConv::MemConvUint8 as u32
            );
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 5 == MvexRegMemConv::MemConvSint8 as u32
            );
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 6 == MvexRegMemConv::MemConvUint16 as u32
            );
            const _: () = assert!(
                MvexRegMemConv::MemConvNone as u32 + 7 == MvexRegMemConv::MemConvSint16 as u32
            );
            ...
            if instruction.is_mvex_eviction_hint() {
                if mvex.can_use_eviction_hint() {
                    ...
                    encoder.set_error_message_str(
                        "This instruction doesn't support eviction hint (`{eh}`)",
                    );
                }
            }
        } else {
            if instruction.is_mvex_eviction_hint() {
                encoder.set_error_message_str(
                    "Only memory operands can enable eviction hint (`{eh}`)",
                );
            }
            if conv == MvexRegMemConv::None {
                ...
                if rc == RoundingControl::None {
                    ...
                    if (this.base.enc_flags3 & EncFlags3::ROUNDING_CONTROL) == 0 {
                        encoder.set_error_message_str(
                            "The instruction doesn't support rounding control",
                        );
                    } else {
                        const _: () = assert!(RoundingControl::RoundToNearest as u32 == 1);
                        const _: () = assert!(RoundingControl::RoundDown as u32 == 2);
            ...
            } else if conv >= MvexRegMemConv::RegSwizzleNone
                && conv <= MvexRegMemConv::RegSwizzleDddd
            {
                if instruction.suppress_all_exceptions() {
                    ...
                    encoder
                        .set_error_message_str("Can't use rounding control with register swizzles");
                }
                b |= ((conv as u32).wrapping_sub(MvexRegMemConv::RegSwizzleNone as u32) & 7) << 4;
            } else {
                encoder.set_error_message_str(
                    "Register operands can't use memory up/down conversions",
                );
            }
        }
