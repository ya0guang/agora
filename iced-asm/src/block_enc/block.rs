...
impl Block {
    ...
    pub(super) fn alloc_pointer_location(&mut self) -> Rc<RefCell<BlockData>> {
        let data = Rc::new(RefCell::new(BlockData {
            data: 0,
            address: 0,
            address_initd: false,
            is_valid: true,
    ...
    pub(super) fn initialize_data(&mut self, base_addr: u64) {
        ...
        let mut addr = base_addr.wrapping_add(self.alignment).wrapping_sub(1)
    ...
    pub(super) fn write_data(&mut self) -> Result<(), IcedError> {
        ...
        for _ in 0..self
            .valid_data_address_aligned
            .wrapping_sub(self.valid_data_address)
        {
