...
deployment_tx_hash = "0x5f014e3df73fe7ddb4ef751f775d60e640fe3eb7b98fab403d06ff40958179a4"
deployment_tx = w3.eth.get_transaction(deployment_tx_hash)
print(f"Deployment transaction details: {deployment_tx_hash} with gas ({deployment_tx['gas']}) is mined at block {deployment_tx['blockNumber']}")
...
print(f"Publishing transaction details: {publish_tx_hash} with gas ({publish_tx['gas']}) is mined at block {publish_tx['blockNumber']}")
...
print(f"Verifying transaction details: {verify_tx_hash} with gas ({verify_tx['gas']}) is mined at block {verify_tx['blockNumber']}")
...
print(f"Verifying ECDSA transaction details: {ecdsa_tx_hash} with gas ({ecdsa_tx['gas']}) is mined at block {ecdsa_tx['blockNumber']}")
