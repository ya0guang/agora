...
impl GenericMemCell<SubRegister> {
    pub fn take_address(&self) -> Result<SSExpr> {
        ...
        let segmented_term: Option<GenericExpr<_>> = match self.segment_reg.register() {
            Register::GS | Register::FS => match base_index_scale_term {
                None => {
                    unreachable!("Segmented term without base index scale term should not happen")
                }
                Some(b) => Some(expr!(
                    GenericExpr::Var(self.segment_reg.into()),
                    bv!("bvadd"),
                    b
                )),
            },
            _ => base_index_scale_term,
        };
        let final_term = if self.displacement == 0 {
            segmented_term
        } else {
            match segmented_term {
...
impl TryFrom<UsedMemory> for MemCell {
    ...
    fn try_from(value: UsedMemory) -> Result<Self, Self::Error> {
        ...
        // This is a special handling on displacement with the pattern: [ebp - 0x10],
        // where iced-x86 would treat the displacement as 0xfffffff0 (in 32-bit mode).
        let disp = match value.base().is_gpr32() {
            true => value.displacement() as i32 as i64,
            false => value.displacement() as i64,
        };

        let mem = MemCellBuilder::new()
            .base_reg(value.base())
            .index_reg(value.index())
            .segment_reg(value.segment())
            .displacement(disp)
...
impl MemCellBuilder {
    ...
    pub fn segment_reg(mut self, segment_reg: Register) -> Self {
        self.v.segment_reg = segment_reg;
        self
    }
